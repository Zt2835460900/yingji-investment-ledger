  ...
  return withPaperWriteLock(async () => {
    try {
      ...
      if (action === "createAccount") {
        ...
        const accountId = Number(body.accountId);
        const amountUnits = decimalToUnits(body.amount, MONEY_SCALE);
        if (!Number.isInteger(accountId) || accountId <= 0)
          throw new Error("请选择模拟账户");
        if (amountUnits <= 0)
          throw new Error("增加金额必须大于 0");
        if (amountUnits > 100_000_000 * MONEY_SCALE)
          throw new Error("单次增加金额不能超过 1 亿元");
        const account = await d1
          .prepare("SELECT initial_cash_units FROM paper_accounts WHERE id = ?")
          .bind(accountId)
          .first<{ initial_cash_units: number }>();
        if (!account) throw new Error("模拟账户不存在");
        const newInitial = account.initial_cash_units + amountUnits;
        if (newInitial > 1_000_000_000 * MONEY_SCALE)
          throw new Error("模拟账户总资金不能超过 10 亿元");
        await d1
          .prepare("UPDATE paper_accounts SET initial_cash_units = ? WHERE id = ?")
          .bind(newInitial, accountId)
          .run();

      } else if (action === "deleteAccount") {
        const accountId = Number(body.id);
        if (!Number.isInteger(accountId) || accountId <= 0)
          throw new Error("模拟账户不存在");
        const account = await d1
          .prepare("SELECT id FROM paper_accounts WHERE id = ?")
          .bind(accountId)
          .first<{ id: number }>();
        if (!account) throw new Error("模拟账户不存在");
        await d1
          .prepare("DELETE FROM paper_trades WHERE account_id = ?")
          .bind(accountId)
          .run();
        await d1
          .prepare("DELETE FROM paper_accounts WHERE id = ?")
          .bind(accountId)
          .run();
