...
export function PaperTrading({
  ...
  const addCashHandler = async () => {
    ...
    if (amount > 100_000_000) {
      setError("单次增加金额不能超过 1 亿元");
      return;
    }
    const ok = await post(
      { action: "addCash", accountId: activeId, amount: addCashAmount },
      `模拟资金已增加 ¥${money(amount)}`,
    );
    if (ok) setAddCashAmount("");
  };
  ...
  const lookup = useCallback(
    async (
      ...
      if (!code) {
        invalidateLookup();
        setMatched(null);
        setLookupNote("请输入股票、ETF、基金或美股代码");
        return null;
      }
      if (!isCompletePaperCode(code)) {
        ...
        setLookupNote("请输入完整的基金/沪深A股代码或美股代码");
        return null;
      }
      if (
        !force &&
  ...
  return (
    <div className="paper-page">
      ...
      <section className="panel paper-add-cash">
        <input
          type="number"
          className="paper-add-cash-input"
          placeholder="增加金额（元）"
          value={addCashAmount}
          onChange={(e) => setAddCashAmount(e.target.value)}
          onKeyDown={(e) => {
            if (e.key === "Enter") void addCashHandler();
          }}
          disabled={busy || !active}
        />
        <button
          className="paper-add-cash-button"
          disabled={busy || !active || !addCashAmount}
          onClick={() => void addCashHandler()}
        >
          {busy ? "处理中…" : "增加模拟资金"}
        </button>
      </section>
        ...
        <section className="panel paper-order-panel">
          ...
          <div className="paper-order-form">
            ...
            <label className="paper-code-field">
              ...
              <small>
                {lookupNote ||
                  "输入代码后读取名称、类型与最新公开价格；支持沪深 A 股与美股"}
              </small>
            </label>
            <label>
              <span>成交日期</span>
