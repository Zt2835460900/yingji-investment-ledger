    ...
    <section className="panel topup-advisor">
      <div className="feature-panel-head">
        <div>
          ...
          <h2>有一笔新资金，怎么分配</h2>
          <p>
            只建议买入低于目标的产品，不会建议卖出。以下按上方已保存的目标比例计算。
          </p>
        </div>
      ...
      <div className="topup-plan-summary" aria-label="本次资金安排">
        <div>
          ...
          <small>
            {explicitCashTargetBps === undefined ? "按旧目标补齐" : "已保存现金目标"}{" "}
        ...
        <div className="feature-empty">
          ...
          <span>
            {plan.requestedAmount <= 0
              ? "请输入大于 0 的可投入金额。"
              : productTargetBps <= 0
                ? "已保存的产品目标全部为 0，这笔资金会继续保留为现金。"
                : plan.reservedCashAmount >= plan.requestedAmount
                  ? "这笔资金需要先满足现金目标，因此暂不建议买入产品。"
                  : "当前产品已达到或超过目标，没有需要补仓的缺口；未分配部分会保留为现金。"}
          </span>
        </div>
      )}
      <div className="topup-footnote">
        <span>
          已分配 ¥{money(plan.allocatedAmount)} / ¥{money(plan.requestedAmount)}
        </span>
        {cashWithoutProductGap > 0.005 ? (
          <span>保留现金后有 ¥{money(cashWithoutProductGap)} 未分配资金</span>
        ) : null}
        <span>
          修改上方目标后请先保存，再查看新方案。算法只纠偏配置，不预测涨跌，不构成投资建议。
        </span>
      </div>
    </section>
  );
...
type FeeChannel = "EASTMONEY" | "DIRECT" | "NONE";
...
export function DcaSimulator({
  ...
  const [investmentDay, setInvestmentDay] = useState("8");
  const [allocations, setAllocations] = useState<DcaAllocation[]>(() => [
    {
      instrumentId: String(eligible[0]?.id ?? ""),
      monthlyAmount: "1000",
      initialAmount: "0",
      feeChannel: "EASTMONEY",
    },
  ]);
  const [historyPeriod, setHistoryPeriod] = useState<HistoryPeriod>("36");
  const [customStartDate, setCustomStartDate] = useState("");
  const [customEndDate, setCustomEndDate] = useState("");
  const [result, setResult] = useState<DcaPortfolioComparison | null>(null);
  ...
  const allocationFeeRate = (allocation: DcaAllocation) => {
    const instrument = eligible.find(
      (item) => String(item.id) === allocation.instrumentId,
    );
    if (!instrument || allocation.feeChannel === "NONE") return 0;
    const bps =
      allocation.feeChannel === "EASTMONEY"
        ? instrument.eastmoney_fee_bps ?? instrument.buy_fee_bps ?? 0
        : instrument.buy_fee_bps ?? 0;
    return bps / 10_000;
  };
  ...
  const runSimulation = async () => {
    const validAllocations = allocations.filter((allocation) => {
      ...
      return instrument && Number(allocation.monthlyAmount) > 0;
    });
    if (!validAllocations.length) {
      setError("请至少添加一个产品，并填写大于 0 的每月投入金额");
      return;
    }
    setLoading(true);
    setError("");
    try {
      const productResults = await Promise.all(
        validAllocations.map(async (allocation) => {
          ...
          const response = await fetch(`/api/fund-history?code=${instrument.code}`, {
            cache: "no-store",
          });
          ...
          if (!response.ok || !history.points?.length)
            throw new Error(`${instrument.name}：${history.error || "历史净值读取失败"}`);
          ...
          return {
            ...
            history,
            orderedPoints,
            result: simulateDcaVsLumpSum(history.points, {
              monthlyAmount: Number(allocation.monthlyAmount),
              initialAmount: Number(allocation.initialAmount || 0),
              investmentDay: Number(investmentDay),
              buyFeeRate: allocationFeeRate(allocation),
      ...
      const simulation = combineDcaComparisons(
        productResults.map((item) => ({
          instrumentId: item.instrument.id,
          name: item.instrument.name,
          monthlyAmount: Number(item.allocation.monthlyAmount),
          result: item.result,
        })),
      );
      setResult(simulation);
      setHistoryDataset({
        earliestDate: productResults.map((item) => item.orderedPoints[0].date).sort().at(-1) ?? "",
        latestDate: productResults.map((item) => item.history.latestDate ?? item.orderedPoints.at(-1)?.date ?? "").sort()[0] ?? "",
        sourceLabel: `${productResults.length} 个产品 · 公开净值`,
        returnMethod: productResults.every((item) => item.history.returnMethod === "TOTAL_RETURN") ? "TOTAL_RETURN" : "MIXED",
        availableMonths: Math.min(...productResults.map((item) => new Set(item.orderedPoints.map((point) => point.date.slice(0, 7))).size)),
      });
    } catch (caught) {
      setResult(null);
      setHistoryDataset(null);
  ...
  const projectionScenarios = useMemo(() => {
    try {
      const base = Number(annualReturn) / 100;
      const options = [
        { label: "保守", rate: Math.max(-0.99, base - 0.03), tone: "down" },
        { label: "基准", rate: base, tone: "base" },
        { label: "乐观", rate: Math.min(1, base + 0.03), tone: "up" },
      ] as const;
      return options.map((scenario) => ({
        ...scenario,
        result: projectLongTermDca({
          monthlyAmount: Number(monthlyAmount),
          years: Number(projectionYears),
  ...
  const historyLimited = Boolean(
    result &&
      (customRangeLimited ||
        result.components.some((component) => component.result.limitedByHistory)),
  );
  const actualYears = result
    ? Math.max(
        0,
        (Date.parse(`${result.endDate}T00:00:00Z`) -
          Date.parse(`${result.startDate}T00:00:00Z`)) /
          86_400_000 /
          365,
      )
    : 0;
  const totalInitialAmount = allocations.reduce(
    (sum, allocation) => sum + Math.max(0, Number(allocation.initialAmount || 0)),
    0,
  );
      ...
      {mode === "history" ? (
        <div className="simulator-workspace">
          <div className="simulator-controls historical-controls">
            <div className="dca-allocation-head">
              <div>
                <span>本次定投组合</span>
                <strong>一次回测多个产品，各自独立计算费率、份额与收益</strong>
              </div>
              <button
                type="button"
                onClick={() => {
                  const next =
                    eligible.find(
                      (instrument) =>
                        !allocations.some(
                          (item) => item.instrumentId === String(instrument.id),
                        ),
                    ) ?? eligible[0];
                  if (!next) return;
                  setAllocations((current) => [
            ...
            <div className="dca-allocation-list">
              {allocations.map((allocation, index) => {
                const allocationInstrument = eligible.find(
                  (item) => String(item.id) === allocation.instrumentId,
                );
                return (
                  <div className="dca-allocation-row" key={`${allocation.instrumentId}-${index}`}>
                    <label className="dca-product-field">
                      <span>产品 {index + 1}</span>
                      <select
                        value={allocation.instrumentId}
                        onChange={(event) =>
                          updateAllocation(index, { instrumentId: event.target.value })
                        }
                      >
                    ...
                    <label>
                      ...
                      <input
                        ...
                        min="1"
                        inputMode="decimal"
                        value={allocation.monthlyAmount}
                        onChange={(event) =>
                          updateAllocation(index, { monthlyAmount: event.target.value })
                        }
                      />
                    </label>
                    <label>
                      ...
                      <input
                        ...
                        min="0"
                        inputMode="decimal"
                        value={allocation.initialAmount}
                        onChange={(event) =>
                          updateAllocation(index, { initialAmount: event.target.value })
                        }
                      />
                    </label>
                    <label>
                      <span>申购渠道 / 费率</span>
                      <select
                        value={allocation.feeChannel}
                        onChange={(event) =>
                          updateAllocation(index, {
                            feeChannel: event.target.value as FeeChannel,
                          })
                        }
                      >
                        <option value="EASTMONEY">
                          天天基金（{((allocationInstrument?.eastmoney_fee_bps ?? allocationInstrument?.buy_fee_bps ?? 0) / 100).toFixed(3)}%）
                        </option>
                        <option value="DIRECT">
                          基金公司直销（{((allocationInstrument?.buy_fee_bps ?? 0) / 100).toFixed(3)}%）
                        </option>
                        <option value="NONE">不计申购费（仅对照）</option>
                      </select>
                    </label>
                    <button
                      type="button"
                      className="dca-remove-product"
            ...
            <label>
              ...
              <select
                value={investmentDay}
                onChange={(event) => {
                  setInvestmentDay(event.target.value);
                  clearHistoricalResult();
                }}
              >
            ...
            <div className="simulator-result historical-result">
              <div className="simulator-period-grid">
                ...
                <div>
                  ...
                  <small>{result.components.length} 个产品 · {result.executionCount} 笔计划投入 · 约 {actualYears.toFixed(1)} 年</small>
                </div>
              </div>

              {historyLimited && (
                <div className="simulator-history-warning" role="status">
                  <strong>组合中至少一个产品的历史数据不足以覆盖你请求的完整区间</strong>
                  <span>
              ...
              <div className="simulator-detail-grid" aria-label="定投回测关键指标">
                <article>
                  <span>年化 XIRR</span>
                  <strong>{result.dca.xirr === null ? "—" : percentText(result.dca.xirr)}</strong>
                  <small>按每笔实际现金流与期末市值年化</small>
                </article>
                <article>
                  <span>策略最大回撤</span>
                  <strong className={result.dca.maxDrawdown < 0 ? "down" : ""}>
                    {percentText(result.dca.maxDrawdown)}
                  </strong>
                  <small>按逐日策略市值，不是单纯基金净值</small>
                </article>
                <article>
                  <span>累计申购费</span>
                  <strong>¥{money(result.dca.totalFees)}</strong>
                  <small>按每个产品的渠道费率分别从预算中扣除</small>
                </article>
                <article>
                  <span>组合产品数</span>
                  <strong>{result.components.length} 项</strong>
                  <small>下方可查看各产品的独立投入与期末市值</small>
                </article>
              </div>
                ...
                {result.components.map((component) => (
                  <article key={component.instrumentId}>
                    <div>
                      ...
                      <span>每月 ¥{money(component.monthlyAmount)} · {component.result.executionCount} 次</span>
                    </div>
                    <div>
                      <span>期末市值</span>
                    ...
                    <div>
                      <span>年化 XIRR</span>
                      <b>{component.result.dca.xirr === null ? "—" : percentText(component.result.dca.xirr)}</b>
                    </div>
                  </article>
                ))}
              </div>
              ...
              <div className="simulator-meta">
                <span>{historyDataset.sourceLabel}</span>
                <span>最新净值日期 {historyDataset.latestDate}</span>
                <span>每月 {investmentDay} 日起的首个可用净值执行</span>
                <span>{totalInitialAmount > 0 ? `首笔额外投入合计 ¥${money(totalInitialAmount)}` : "未设置首笔额外投入"}</span>
                <span>现金分红按除息日再投资计入</span>
                <span>已按各产品渠道计申购费；未计赎回费与税费</span>
              </div>
        ...
        <div className="simulator-workspace projection-workspace">
          ...
          {projection && (
            <div className="simulator-result projection-result">
              ...
              <div className="projection-scenarios" aria-label="长期情景区间">
              ...
              {goalCalculation && (
                <div className="projection-goal-card">
                  <div>
                  ...
                  <button
                    type="button"
                    onClick={() => setMonthlyAmount(String(goalCalculation.requiredMonthlyAmount))}
                  >
                    应用到月投
                  </button>
                </div>
