...
function Ledger({
  ...
  onEntry: () => void;
  onEdit: (entry: PortfolioData["ledger"][number]) => void;
  onDelete: (id: number) => void;
}) {
  const exportLedger = () => {
    const header =
...
function Allocation({
  ...
  const currentCashValue = Math.max(0, data.metrics.cash ?? 0);
  ...
  const hiddenTargetRows = unheldTargetRows.filter((row) =>
    row.savedTarget > 0 ? !showLegacyTargets : !showOtherProducts,
  );
  ...
  const revealHiddenTargets = () => {
    setShowLegacyTargets(true);
    setShowOtherProducts(true);
    window.setTimeout(() => {
      const target = document.getElementById(
        legacyRows.length ? "legacy-target-list" : "other-target-list",
      );
  ...
  const useCurrentHoldingsAsDraft = () => {
    if (data.metrics.totalAssets <= 0) return;
    if (
      !confirm(
        "按当前资产生成目标比例草稿？\n\n产品按当前市值占总资产的比例填写，现有现金比例会被保留。没有持有的旧目标会设为 0。这里只生成草稿，不会自动保存，也不会发生任何买卖。",
      )
    )
      return;
    ...
    const exactProductTargets = heldRows.map((row, index) => {
      const exact = (row.currentValue / data.metrics.totalAssets) * 10_000;
      const bps = Math.floor(Math.max(0, exact));
      return { row, index, bps, remainder: exact - bps };
    });
    let bpsLeft = Math.max(
      0,
      productBudgetBps -
        exactProductTargets.reduce((sum, item) => sum + item.bps, 0),
    );
    const remainderOrder = [...exactProductTargets].sort(
      (a, b) => b.remainder - a.remainder || a.index - b.index,
    );
    for (let index = 0; bpsLeft > 0 && remainderOrder.length; index += 1) {
      remainderOrder[index % remainderOrder.length].bps += 1;
      bpsLeft -= 1;
    }
    exactProductTargets.forEach(({ row, bps }) => {
      nextDrafts[row.instrument.id] = (bps / 100).toFixed(2);
    });
    ...
    <div className="allocation-page">
      ...
      <section className="panel allocation-target-panel">
        ...
        >
          <div>
            <strong>
              {targetValuesValid && productTargetBps <= 10_000
                ? `产品 ${productTarget.toFixed(1)}% + 现金 ${cashTarget.toFixed(1)}% = 100%`
                : productTargetBps > 10_000
                  ? `产品目标合计 ${productTarget.toFixed(1)}%，已超过 100%`
                  : "请检查产品目标比例"}
            </strong>
            <span>
              {!targetValuesValid
                ...
                : targetTotalIsValid
                  ...
                  : hiddenTargetRows.length > 0
                    ? `产品目标已超出 ${(productTarget - 100).toFixed(1)}%；还有 ${hiddenTargetRows.length} 项未持有目标折叠在下方，可能仍在占用比例`
                    : `产品目标已超出 ${(productTarget - 100).toFixed(1)}%，请调低部分目标`}
            </span>
          </div>
          <div className="target-total-actions">
            {!targetTotalIsValid && hiddenTargetRows.length > 0 && (
              <button
                type="button"
                className="secondary-button"
                onClick={revealHiddenTargets}
              >
                展开相关目标
              </button>
            )}
        ...
        <div className="cash-target-card" aria-label="现金目标">
          <div className="cash-target-name">
            ...
            <div>
              ...
              <small>产品目标未用完的比例会自动留作现金</small>
            </div>
          </div>
          <div className="target-current-value">
            <span>当前现金</span>
            <strong>¥{money(currentCashValue)}</strong>
            <small>占总资产 {currentCashPercent.toFixed(1)}%</small>
          </div>
          <div className="cash-target-readonly">
            <span>自动目标</span>
            <strong>{cashTarget.toFixed(2)}%</strong>
          </div>
          <div
            className={`target-gap ${
          ...
          >
            <strong>
              {Math.abs(cashGap) <= 0.05
                ? "现金已达到目标"
                : cashGap > 0
                  ? `现金还差 ${cashGap.toFixed(1)}%`
                  : `现金超出 ${Math.abs(cashGap).toFixed(1)}%`}
            </strong>
            <span>
              {Math.abs(cashGap) <= 0.05
                ? "当前现金比例与目标一致"
                : cashGap > 0
                  ? `按当前总资产折算，约需保留 ¥${money(cashGapAmount)}`
                  : `按当前总资产折算，约有 ¥${money(cashGapAmount)} 可待配置`}
            </span>
          </div>
        </div>
...
function ModalForm({
  ...
  const [form, setForm] = useState<Record<string, string>>({
    ...
    instrumentCode:
      type === "entry" && editingEntry
        ? (initialEntryInstrument?.code ?? "")
        : type === "plan"
          ? (initialPlanInstrument?.code ?? "")
          : (data.instruments[0]?.code ?? ""),
    preferredProductType:
      (initialEntryInstrument ?? initialPlanInstrument)?.product_type ===
      "STOCK"
        ? "STOCK"
        : "FUND",
  ...
  const [confirmationBusinessDays, setConfirmationBusinessDays] = useState<
    number | null
  >(null);
  const editingCalculatedAmount = editingEntry
    ? (editingEntry.quantity_units / 1_000_000) *
      (editingEntry.price_units / 1_000_000)
    : 0;
  const editingSavedAmount = editingEntry
    ? editingEntry.gross_amount_units / 10_000
    : 0;
  ...
  useEffect(() => {
    ...
    const isOriginalEditedEntry =
      Boolean(editingEntry) &&
      editingEntry?.instrument_id === existing?.id &&
      editingEntry?.trade_date === form.tradeDate &&
      initialEntryInstrument?.code === code &&
      !priceIsAuto;
    if (isOriginalEditedEntry) {
      setResolvedInstrument(initialEntryInstrument);
      setLookupBusy(false);
      setLookupNote(
        `已载入原流水：份额、净值和金额都可以修改；金额清空后会恢复自动计算`,
      );
      return;
    }
